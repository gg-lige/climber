...
public class FPTree {
	 ...
	 * @return
	 */
	private TreeNode buildSubTree(List<List<String>> transRecords, final Map<String, Integer> freqMap, final Map<String, TreeNode> headers) {
		...
		for (Entry<List<String>, Integer> entry : frequentMap.entrySet()) {
			...
			for (StrongAssociationRule rule : rules) {
				if (frequentMap.containsKey(rule.condition)) {
					...
					System.err.println(rule.condition + " is not a frequent pattern, however " + items + " is a frequent pattern");
				}
			}
		}
		return rect;
	...
	public static void main(String[] args) throws IOException {
		...
		for (StrongAssociationRule rule : rules) {
			...
			bw.write(rule.condition + "->" + rule.result + "\t" + dfm.format(rule.support) + "\t" + dfm.format(rule.confidence));
			bw.newLine();
		}
		bw.close();
	}
	 ...
	 * @throws IOException
	 */
	public static List<StrongAssociationRule> execute(List<List<String>> trans, Set<String> decideAttr) throws IOException {
